...
const TeachersHomeScreen = () => {
  ...
  const clubschoolname = route.params.schoolclubname
    ...
    for(let i = 9; i < schoolname1.length; i++){
        if (schoolname1[i] != "\"" && schoolname2[i+1] != "}") {
            schoolname2 += schoolname1[i]
        }
        else {
            break
        }
  ...
  const signOut = async() => {
    ...
    return (
      ...
      <ScrollView>
       ...
       <Text style = {styles.infofont}> Classes {schoolname2} teaches: {'\n'}{president} </Text>
       <Text style = {styles.infofont}>Description of Teacher:  {'\n'} {descriptionofclub}  </Text>
...
export default TeachersHomeScreen;